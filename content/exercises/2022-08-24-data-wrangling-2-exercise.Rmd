...
Copy the below code into a chunk at the top. The `penguins_long`, `penguins_wide`, `TX_population`, and `TX_area` tables are for later exercises.
...
First, we will be working with the dataset `penguins` containing data on individual penguins on Antarctica.
<pre class="r">
<code class="language-r">
`r ''````{r}
penguins
...
```{r no-group-example, echo=FALSE, eval=TRUE}
penguins %>%
  print()
```
...
```{r with-group-example, echo=FALSE, eval=TRUE}
penguins %>%
  group_by(species) %>%
  print()
```
...
Try `right_join()` and `inner_join()` alsoSee if you can describe what each join function does and how they differ from one another.