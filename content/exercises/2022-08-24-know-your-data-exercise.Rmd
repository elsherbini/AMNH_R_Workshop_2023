...
```{r case_when-demo, eval=TRUE, echo = TRUE}
NCbirths %>%
  mutate(
...
```{r read_csv-demo, eval=TRUE, echo = TRUE}
simple_csv <- "
a, b, c, d
dog,#N/A,2,3
,3,NA,2
...
```{r read_csv-demo2, eval=TRUE, echo = TRUE}
read_csv(simple_csv)
```
...
```{r read_csv-replace_at-demo, eval=TRUE, echo = TRUE}
library(naniar)
read_csv(simple_csv) %>%
...
```{r mean-with-NA-demo, eval=TRUE, echo = TRUE}
...
```{r missing-scatter-demo, eval=TRUE, echo = TRUE}
