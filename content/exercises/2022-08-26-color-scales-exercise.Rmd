...
```{r temps_months, eval=TRUE, echo = TRUE}
temps_months
```

As a challenge, try to create this above table yourself using `group_by()` and `summarize()` like we learned about Wednesday. If you are having trouble, the solution is at the end of this page.

```{r temp_months_by_user, echo=TRUE}

temps_months <- read_csv("https://wilkelab.org/SDS375/datasets/tempnormals.csv") %>%
  group_by(___) %>%
  summarize(___) %>%
  mutate( # keep this part the same, to recode the month name as a 3 letter code
    month = factor(
      month_name,
      levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
    ),
...
```{r swatchplot, echo = TRUE, eval=TRUE, fig.height = 3.5}
...
```{r swatchplot2, echo = TRUE, eval=TRUE, fig.height = 3.5}
# Okabe-Ito colors
